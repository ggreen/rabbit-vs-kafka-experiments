...
public interface JobStatsRepository {
    ...
    /*
    """SELECT new com.test.book.BookStats(b.author, b.title, count(b))
                from Book b where b.title = :#{#filter.title} and b.author= :#{#filter.author}
                """
     */
    public static final String QUERY = """
            select
                new  experiments.streaming.batch.reporting.domain.JobStats(
                job_name,
            	count(*) total_cnt,
            ...
            (select ji.job_name as job_name, je.end_time - je.start_time as total_time,
                 (( select step.write_count from batch_step_execution step where step.job_execution_id= je.job_execution_id)
